rankall <- function(outcome, num = "best") {
      ## Read outcome data
      data <- read.csv("~/Desktop/data/outcome-of-care-measures.csv", colClasses = "character")
      ...
      states <- cbind(sort(c(unique(data$State))),1:length(unique(data$State)))
      rankDF <- as.data.frame(matrix(ncol=2,nrow=nrow(states)))
      names(rankDF)=c("hospital","state")
      row.names(rankDF)<-states[,1]
      for (j in 1:nrow(states)){
            hospitals <- character( length=0)
            mortalityRates <- integer( length = 0)
            for (i in 1:nrow(data))
            {
                  if (data[i,7]==states[j])
                  {
            ...
            hospitalsMat <- matrix(hospitals,ncol=1)
            hospMatrix <- cbind(hospitalsMat,mortalityRates)
            orderedHospitals <- matrix(hospMatrix[order(mortalityRates,hospitals),]
                                       ,ncol=ncol(hospMatrix),nrow=nrow(hospMatrix))
